...
\begin{itemize}
    ...
    \item \textbf{Approvato}: Questa fase coincide con il completamento della parte di "\emph{Verifica}" avvenuta con successo nella quale il verificatore certifica la correttezza. Il documento verrà dunque consegnato al \emph{\markg{responsabile}} il quale lo approverà per il rilascio esterno.
\end{itemize}
...
\textbf{ESTERNO}
\begin{itemize}
    \item \textbf{[ AdR ] - Analisi dei Requisiti}: Questo documento viene redatto agli \emph{Analisti} dopo che questi ultimi hanno analizzato il capitolato e interagito con il proponente. All’interno dell’analisi dei requisiti saranno esposti tutti i \markg{requisiti} del progetto, inclusi i diagrammi di interazione con l’utente ed i casi d’uso;
...
\begin{itemize}
    ...
    \item \markg{TexMaker}\footnote{\href{http://www.xm1math.net/texmaker/}{http://www.xm1math.net/texmaker/}};
\end{itemize}
...
Per garantire la qualità del lavoro, gli Amministratori hanno suddiviso il lavoro in vari \markg{processi} che sono stati poi riportati nel \emph{Piano di Qualifica}. Dovrà essere rispettata la notazione \textbf{PROC[value]}  dove \textbf{value} indica il codice univoco del \markg{processo} tramite un numero intero a tre cifre che parte da 1 ed incrementa per ogni unità.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{S}: per indicare il \markg{software}.
    \end{itemize}
\end{itemize}
\begin{itemize}
...
\subsubsection{Controllo di qualità di \markg{processo}}
La qualità dei processi verrà garantita dal metodo \markg{PDCA}, descritto nell'appendice A. Grazie a tale metodo si potrà ottenere un miglioramento continuo della qualità di tutti i processi, inclusa la \markg{verifica}. Per ottenere la qualità dei processi bisogna:
\begin{itemize}
    \item Definire il \markg{processo} affinchè esso sia controllabile;
    \item Controllare il \markg{processo} in funzione del raggiungimento di un alto livello di \markg{efficacia} ed \markg{efficienza};
    \item Usare strumenti di valutazione quali \markg{PDCA} e \markg{SPICE}.
\end{itemize}
...
\paragraph{Struttura della \markg{repository}}%TODO mi da errore da qui: \\[0.4cm]
...
\paragraph{Ciclo di vita dei \markg{branch}}\mbox{}\\[0.4cm]
...
\paragraph{Aggiornamento della \markg{repository}}\mbox{}\\[0.4cm]
...
\begin{itemize}
    ...
    \item Client git: sono state utilizzate le applicazioni GitKraken (per sistemi operativi basati su Linux) e \markg{GitHub} Desktop (per sistemi MacOS e Windows), oltre agli strumenti da linea di comando.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Controllare il \markg{processo}}\\ Con l'obbiettivo di ottenere \markg{efficacia} ed \markg{efficienza}  
    \item \textbf{Usare strumenti di valutazione}\\ \markg{PDCA} e \markg{SPICE}
\end{itemize}
\subsection{Metriche qualità per il \markg{processo}}
\label{sec:qualita_processo}
Verranno utilizzate le seguenti metriche per valutare l’\markg{efficienza} e l’\markg{efficacia} dei \markg{processi}.
...
Media dei commit effettuati settimanalmente nelle diverse \markg{repository} utilizzare dal gruppo. La misurazione avviene grazie ad un \markg{bot} collegato a \markg{GitLab} ed integrato su \markg{Slack} in grado di leggere i log degli eventi sulla \markg{repository}.
...
\begin{itemize}
    \item \textbf{MPRC006.1 - Percentuale test passati}: indica la percentuale di test passati ed è utile per capire a che punto ci si trova nella fase di sviluppo del \markg{software}. La formula per il calcolo del \textit{PTP} è la seguente:
    \begin{center}
        \begin{displaymath}
            PTP = \frac{TP}{TE} * 100
        \end{displaymath}
    ...
    \item \textbf{MPRC006.2 - Percentuale test falliti}: indica la percentuale di test falliti ed è utile per capire a che punto ci si trova nella fase di sviluppo del \markg{software}. La formula per il calcolo del \textit{PTF} è la seguente:
    \begin{center}
        \begin{displaymath}
            PTF = \frac{TF}{TE} * 100
        \end{displaymath}
    ...
    \begin{center}
        ...
        \end{displaymath}
    \end{center}
    Dove DT indica il numero totale di difetti trovati nella fase di test mentre DTU indica la somma dei difetti trovati nei test e quelli trovati durante l'uso del \markg{software};
...
\begin{itemize}
    ...
    \item \textbf{MPRC007.2 - Difetti per \markg{requisito}}: indica il numero di difetti trovati nel test del \markg{requisito} e fornisce informazioni sullo stato di soddisfacimento del \markg{requisito}. In caso di zero difetti, il \markg{requisito} viene considerato senza errori e dunque soddisfatto. La misurazione si mostra con una tabella avente nella prima colonna il nome del \markg{requisito} mentre nella seconda i relativi difetti.
\end{itemize}

\subsection{Metriche qualità per i documenti}
\label{sec:qualita_prodotto}
Verranno utilizzate le seguenti metriche per valutare l’\markg{efficienza} e l’\markg{efficacia} dei prodotti.
...
\subsection{Metriche qualità per il \markg{software}}
...
\subsubsection{MPRDS003 - Percentuale di \markg{failure}}
...
Indica il tempo medio che trascorre tra la richiesta da parte del \markg{software} di una certa risorsa o funzionalità e la restituzione del risultato da parte dell'utente. La formula di misurazione è la seguente:
\begin{center}
    \begin{displaymath}
...
\begin{itemize}
    ...
    \item \textbf{MPRDS014.3 - Branch coverage}: Verificare se tutti i possibili \markg{branch} di un \textit{if} sono stati eseguiti;
    \item \textbf{MPRDS014.4 - Lines coverage}: Verificare se ogni linea è stata eseguita e/o percorsa;
\end{itemize}
Verrà utilizzato il tool JaCoCo\footnote{\href{https://www.jetbrains.com/help/idea/code-coverage.html}{https://www.jetbrains.com/help/idea/code-coverage.html}} che è integrabile all'interno dell'\markg{IDE} IntelliJ IDEA e \markg{Android} Studio per il linguaggio \markg{Java}. Per quanto riguarda \markg{Node.js} si farà affidamento a \footnote{\href{https://mochajs.org/}{https://mochajs.org/}}.
...
\subsubsection{Strumenti}
\begin{itemize}
    \item \textbf{Software}:  per la stesura del codice \markg{Java}, si utilizzeranno gli \markg{IDE} \emph{Intellij \markg{IDEA}} e \emph{\markg{Android} Studio}. Per la stesura del codice javascript per \markg{Node.js} si utilizzerà \emph{Visual Studio Code};
