...
{
	name=Amplify,
	description={\markg{Framework} di \markg{AWS} che facilita la creazione, la configurazione e l’implementazione di applicazioni web, oltre che gestire il \markg{back-end} per dispositivi mobili. Amplify automatizza anche il processo di rilascio dell’applicazione sia per il \markg{front-end}, sia per il back-end, consentendo così la distribuzione di funzionalità in modo più rapido}
}

\newglossaryentry{Cognito}
{
	name=Cognito,
	description={\markg{Framework} di \markg{AWS} che permette di aggiungere strumenti di registrazione, accesso e controllo degli accessi alle app Web e per dispositivi mobili in modo rapido e semplice. Supporta l'accesso con provider di identità social quali Facebook, Google e Amazon e provider di identità aziendali tramite SAML 2.0}
}

\newglossaryentry{AppSync}
{
	name=AppSync,
	description={\markg{Framework} di \markg{AWS} che semplifica lo sviluppo delle applicazioni consentendo di creare un'\markg{API} flessibile per l’accesso, la manipolazione e la combinazione di dati da una o più origini dati. Si tratta di un servizio gestito che utilizza GraphQL che consente alle applicazioni di ottenere in modo semplice i dati di cui hanno bisogno}
}

\newglossaryentry{Android Studio}
{
	name=Android Studio,
	description={\`E un ambiente di sviluppo integrato per lo sviluppo per la piattaforma \markg{Android}. Disponibile gratuitamente sotto licenza Apache 2.0}
}
...
\newglossaryentry{builder pattern}
{
...
\newglossaryentry{singleton pattern}
{
...
\newglossaryentry{factory pattern}
{
